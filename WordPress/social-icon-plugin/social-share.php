...
function enqueue_font_awesome() {
    ...
    wp_enqueue_style('social-share', plugin_dir_url(__FILE__) . 'social-share.css', array(), '1.0.0');
}
...
function social_share_activate() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'socialshare';
...
function social_share_save_data() {
    if (isset($_POST['facebook_url']) && isset($_POST['instagram_url']) && isset($_POST['twitter_url']) && isset($_POST['linkedin_url'])) {
        ...
        wp_redirect(admin_url('admin.php?page=social_share_menu&status=success'));
        exit;
    }
}
